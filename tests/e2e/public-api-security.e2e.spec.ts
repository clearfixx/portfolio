...
function createClientHeaders(label: string) {
  ...
  return {
    ...
    'user-agent':
      `portfolio-security-e2e/${label}/${identity}`,
  }
}

function expectBoundaryHeaders(
...
test.describe('public API security boundary', () => {
  ...
  test('rejects malformed JSON', async ({
    ...
    const response = await request.post(
      ...
      {
        headers:
          createClientHeaders('malformed-json'),
        data: '{"email":',
      },
    )
  ...
  test('rejects an oversized request body', async ({
    ...
    const response = await request.post(
      ...
      {
        headers:
          createClientHeaders('oversized-body'),
        data: JSON.stringify({
          email:
  ...
  test('ignores spoofed proxy IP headers and enforces the contact limit', async ({
    request,
  }) => {
    const clientHeaders =
      createClientHeaders('spoofed-proxy')
    let limitedResponse:
      | APIResponse
      | undefined
    ...
    if (!limitedResponse) {
      throw new Error(
        'The rate-limited response was not captured.',
      )
    }
    ...
    expect(headers['retry-after']).toMatch(
      /^\d+$/,
    )
    expect(
    ...
    expect(headers['x-ratelimit-reset']).toMatch(
      /^\d+$/,
    )
    expect(headers['x-ratelimit-policy']).toBe(
      '8;w=600',
    )
  })
})
